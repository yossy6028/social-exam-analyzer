...
class TextAnalyzer:
    ...
    def _detect_sections(self, text: str) -> List[Dict[str, Any]]:
        ...
        section_patterns = [
            # 「一、次の文章を読んで」パターン（最優先）
            (r'[一二三四五六七八九十][、\s]+次の文章を読んで', 'main_section'),
            # 「第一問」などのパターン
        ...
        for pattern, p_type in section_patterns:
            for match in re.finditer(pattern, text, re.MULTILINE):
                all_matches.append((match.start(), match.end(), match.group(), p_type))
        ...
        for start, end, marker, p_type in all_matches:
            # 前のマッチと近すぎる場合はスキップ
            if start - prev_pos < 500:  # 500文字以上離れていること（大問は十分離れているはず）
                continue
                
            # 大問の後に十分なテキストがあることを確認
            remaining_text = text[end:end+500] if end+500 < len(text) else text[end:]
            if len(remaining_text.strip()) < 50:  # 最低50文字のコンテンツ
                continue
                
            selected_matches.append((start, end, marker, p_type))
            prev_pos = start
        
        # 大問を構築
        for i, (start, end, marker, p_type) in enumerate(selected_matches):
            section_start = start
            section_end = selected_matches[i + 1][0] if i + 1 < len(selected_matches) else len(text)
            
            section_text = text[section_start:section_end]
            
            # 文章が含まれていることを確認（出典があるかチェック）
            if self._is_valid_section(section_text):
                sections.append({
